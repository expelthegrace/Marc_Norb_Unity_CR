	...
	void Update () {
        ...
        if (actualTime - lastCar > freq)
        {
            ...
            if (costat == 0)
            {
                ...
                if (Random.Range(0,100) < 50) auto = Instantiate(van, inipos, Quaternion.Euler(0, 0, 0), GameObject.Find("Cotxes").transform);
                else auto = Instantiate(cotxe, inipos, Quaternion.Euler(0, 0, 0), GameObject.Find("Cotxes").transform);

            }
            else
            {
                inipos = new Vector3(GetComponent<Collider>().bounds.min.x - 60, GetComponent<Collider>().bounds.max.y, transform.position.z);
                if (Random.Range(0, 100) < 50) auto = Instantiate(van, inipos, Quaternion.Euler(0, 180, 0), GameObject.Find("Cotxes").transform);
                else auto = Instantiate(cotxe, inipos, Quaternion.Euler(0, 180, 0), GameObject.Find("Cotxes").transform);
